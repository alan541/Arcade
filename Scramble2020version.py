...
while run:
    ...
    for event in pygame.event.get(): 
        ...
        if event.type == pygame.QUIT: 
            ...
            if event.key == pygame.K_SPACE and counter <= 6500:
                shoot()
    ...
    def playerhits1(x):
        ...
        hits = pygame.sprite.spritecollide(cp, x, False)
        if hits:
    ...
    p, L = playerhits1(landscape)
    p, L = playerhits1(defenders)
    p, L = playerhits1(asteroids)
    ...
    def playerhits2(x):
        ...
        if hits:
            ...
            for i in range(3):
                clock.tick(1)
            if counter - 11500 + cp.rect.x + 20 <= 20:
                cp.rect.x -= 30
            else:
                cp.rect.x = wb[counter - 11500 + cp.rect.x + 20].rect.x
                cp.rect.y = wb[counter - 11500 + cp.rect.x + 20].rect.y - 25

        return cp, lives
    
    p, L = playerhits2(wallb)
    p, L = playerhits2(wallt)
